...
namespace Meta.Driver.Model
{
	public class DbSplitStrategy<TDbName, TSplitTable, TReferenceTable, TReferenceField> : DbSplitStrategy
		...
		where TReferenceTable : class, IDbModel, new()
	{
		/// <summary>
		/// 
		/// </summary>
		...
		public DbSplitStrategy(SplitType splitType, object[] accordingValue, string suffix = null) : base(typeof(TReferenceField), typeof(TDbName), typeof(TSplitTable), typeof(TReferenceTable), splitType, accordingValue, suffix)
		{
		}
	}
	public abstract class DbSplitStrategy
	{
		...
		internal DbSplitStrategy()
		{
			...
			SplitTable = EntityHelper.GetTableName(SplitTableType);
			ReferenceTable = EntityHelper.GetTableName(ReferenceTableType);
		...
		public virtual string GetRealTable(object value)
		{
			...
			switch (SplitType)
			{
				...
				case SplitType.EnumEveryValue:
					...
					break;
				case SplitType.GuidEveryFirstLatter:
