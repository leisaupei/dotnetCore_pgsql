...
namespace Meta.xUnitTest
{
	...
	public class TypeMap : BaseTest
	{
		...
		public void Bit()
		{
			var affrows = TypeTest.Update(Guid.Empty).Set(a => a.Bit_type, new BitArray(1, false)).ToRows();
		...
		public void Enum()
		{
			var affrows = TypeTest.Update(Guid.Empty).Set(a => a.Enum_type, EDataState.已删除).ToRows();
			Assert.True(affrows > 0);
		}
