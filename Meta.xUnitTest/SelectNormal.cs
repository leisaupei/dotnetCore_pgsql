...
namespace Meta.xUnitTest
{
	...
	public class SelectNormal : BaseTest
	{
		...
		{
			var info = Student.Select.Where(a => new[] { StuPeopleId1, StuPeopleId2 }.Contains(a.People_id)).ToList();
		...
		{
			var info = Student.Select.Where(a => a.Stu_no == StuNo1).ToOne();
		...
		{
			var info = Student.Select.Where(a => new[] { StuNo1, StuNo2 }.Contains(a.Stu_no)).ToList();
