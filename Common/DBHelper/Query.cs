...
namespace DBHelper
{
	public class Query<T> : QueryHelper<T> where T : class, new()
	{
		...
		#region where
		protected Query<T> WhereOr(string filter, Array values) => base.Where(filter, values) as Query<T>;
		...
		public TResult ToTuple<TResult>(string fields) => ToOne<TResult>(fields);
		public List<TResult> ToTupleList<TResult>(string fields) => ToList<TResult>(fields);
